...
public class TypeFoodController {
	...
	String getCategory(Model model) {
		List<TypeFood> list = service.repository.findAll();
		if (list == null)
			model.addAttribute("list", null);
		else
			model.addAttribute("list", list);
		return "typeFood-list";
	}

	@RequestMapping(path = { "/edit", "/edit/{id}" })
	public String edit(Model model, @PathVariable(name = "id", required = false) Long id){
		if (id != null) {
			TypeFood entity = service.read(id);
			model.addAttribute("entity", entity);
		} else {
			model.addAttribute("entity", new TypeFood());
		}
		return "typeFood-add-edit";
	}
